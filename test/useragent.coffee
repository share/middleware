# These tests have not yet been migrated over from sharejs. They do not work.
...
describe 'UserAgent', ->
  beforeEach (next) ->
    # Stub out backend methods as needed.
    @backend = {}
  ...
  describe.only 'fetch', ->
    beforeEach ->
      @backend.fetch = (cName, docName, callback) =>
        assert.strictEqual cName, 'flowers'
        assert.strictEqual docName, 'lily'
        callback null, {v:10, type:'atype', data:'yellow'}

    it 'calls fetch on backend', (done) ->
      @agent.fetch 'flowers', 'lily', (err, data) =>
        throw Error err if err
        assert.deepEqual data, {v:10, type:'atype', data:'yellow'}
        done()

    it 'calls middleware', (done) ->
      called = 0
      @middleware.use 'fetch', (req) =>
        called++
        assert.equal req.action, 'fetch'
        assert.equal req.cName, 'other_flowers'
        assert.equal req.docName, 'rose'

        assert.strictEqual req.agent, @agent

        # Need to be able to redirect the fetch in middleware.
        req.cName = 'flowers'
        req.docName = 'lily'
    ...
    describe.skip 'with doc filters', ->
